...
本节描述本仓库测试与适配层所约定的 **Python 契约**：实现应放在目录 `src/bpe_tokenizer/`（可导入包名 `llm_from_scratch.bpe_tokenizer`），并通过 `tests/adapters.py` 中的薄封装与 pytest 对齐。
...
建议在 `src/bpe_tokenizer/__init__.py` 中导出 `train_bpe`，以便 `from llm_from_scratch.bpe_tokenizer import train_bpe` 可用。
