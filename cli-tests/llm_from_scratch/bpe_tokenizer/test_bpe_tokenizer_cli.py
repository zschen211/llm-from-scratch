...
# pytest 在收集 `cli-tests/.../test_*.py` 时，可能会把 `cli-tests` 加入 `sys.path`，
# 从而让 `cli-tests/llm_from_scratch/*` 目录遮蔽真实的 `llm_from_scratch` 包。
# 强制把项目根目录插到 sys.path 最前面，确保导入落到真实实现。
...
def test_bpe_tokenizer_cli_encode_decode(run_cli, project_root: Path, tmp_path: Path) -> None:
    ...
    proc = run_cli(
        "cli/llm_from_scratch/bpe_tokenizer/bpe_tokenizer_cli.py",
        [
            "encode",
            "--checkpoint",
            str(ckpt),
